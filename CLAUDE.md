...
- **Environment/Runner**: UV (package manager and runtime)
- **AI Framework**: Claude Agent SDK
- **AI Models**: Sonnet 4.5 (orchestrator), Haiku (5 parallel workers)
- **UI Framework**: Gradio 4.x+ (optional)
- **Database**: SQLite 3.x (tag analytics)
- **Progress Display**: Rich
- **Task Scheduling**: APScheduler + cron
- **External Tools**: yt-thumbs (UV tool for YouTube thumbnails)

## Architecture Notes
- **Main Orchestrator (Sonnet 4.5)**: Manages batching, coordinates subagents, handles state/resume
- **Tag Analyzer Subagent (Haiku)**: Analyzes existing tags, builds frequency map, saves to SQLite
- **Worker Subagents (5x Haiku)**: Parallel processing of individual raindrops (title, thumbnail, description, tags)
- **State Management**: Uses `#processed-by-script` tag to track processed items
- **Backup Strategy**: JSON backups before any modification with undo capabilities
- **API Integration**: Raindrop API wrapper with rate limiting
...
- Store API keys in `secrets.json` (NEVER commit)
  ...
  - `raindrop_api_token`: For Raindrop API access
